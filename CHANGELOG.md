...
### Added
- `src/utility.js` / `src/XChainIndexer.js` — the hub-owned price tables (`price_snapshots`, `oracle_prices`) are read from a dedicated hub DB connection (`this.hubDb`) when `HUB_DB_HOST` / `HUB_DB_NAME` are set, and otherwise fall back to the indexer's own database. That fallback is correct and intended for single-host deployments (where the local DB already holds the synced hub copy), but it was completely silent: a distributed node with the hub DB vars accidentally unset or misconfigured would serve stale or empty price data for fee validation and FIAT dispenser settlement with no error and no log signal, risking ledger divergence from hub-connected peers. The fallback behaviour is unchanged; it is now observable. `XChainIndexer.start()` emits a one-time boot `console.warn` when no hub DB credentials are supplied, and `getFeeOraclePrices()` (`src/utility.js`) emits a one-time `console.warn` (guarded by an instance flag so it does not spam every fee calculation) the first time it actually takes the local-DB price path, naming the function and the `HUB_DB_HOST`/`HUB_DB_NAME` vars so an operator searching logs can immediately identify the misconfiguration. Purely additive — no behaviour, schema, or protocol change; single-host and e2e paths that rely on the fallback are unaffected.
- `src/XChainIndexer.js` — the hub config overlay is now kept live by a polling loop instead of being applied only once at startup. The indexer overlays operational params served by the hub (`ACTIVATION_DELAY_BLOCKS`, `EXPIRATION_FEE_PER_DAY`, `STAKING`) on top of its local config; previously that overlay ran once at boot, so a governance-committed parameter change did not take effect on a running indexer until the process was restarted — the worst-exposed of the hub's config consumers. (Consensus-critical fee inputs `GAS_SCHEDULE` and `GAS_PRICE` are deliberately excluded from the overlay — see Fixed below.) `_startHubConfigPolling()` now re-fetches `getallconfigs` every `HUB_CONFIG_POLL_INTERVAL_MS` (default 60000) and re-applies the overlay only when the hub's committed PBFT sequence (`seq`) advances past the last applied one, so a steady-state poll is a cheap no-op and an unchanged `seq` never re-applies. Against an older hub that returns the bare config map, `seq` stays `0` and the overlay is never re-applied (matching the prior startup-only behaviour), so the change is backward-compatible. The poll timer is `unref`'d and never keeps the process alive. The merge logic was extracted into `_mergeHubParams()` / `_unwrapHubConfigResponse()` with no change to which params are overlaid.
...
- `package.json` — pinned `mathjs` to the exact version `15.2.0` (dropping the `^15.2.0` caret) and added a matching `mathjs: 15.2.0` entry to `overrides`. The bundled `xchain-vm` local dependency pins `mathjs` to exactly `15.2.0`, and both the indexer's consensus amount/fee arithmetic and the VM's deterministic contract math depend on a single shared bignumber implementation. With a caret range, a future `npm install` that resolved `mathjs` to a newer patch (e.g. `15.3.x`) would force npm to nest a second `mathjs` copy inside the `xchain-vm` subtree while the indexer kept the newer one — two implementations running side by side with potential silent precision divergence and no runtime error. The exact pin plus the override removes that hazard: npm can only ever resolve one `mathjs@15.2.0` for the whole tree regardless of how the lockfile evolves. Lockfile regenerated. No source changes.
...
- `src/configs/LTC.js` / `src/configs/DOGE.js` — `STAKING.ACTIVATION_DELAY_BLOCKS` is now calibrated per chain for equivalent reorg protection instead of a flat 6 blocks. The 6-block default was justified by BTC's ~10-minute blocks (~60 minutes of protection), but contract-targeted staking (STAKE v3 / UNSTAKE v1 / DELEGATE v1+v3) runs on every chain and read the same constant — so on LTC (~2.5-min blocks) 6 blocks gave only ~15 minutes and on DOGE (~1-min blocks) only ~6 minutes of reorg protection. LTC is now `24` and DOGE is now `60` (BTC stays `6`), each ≈60 minutes at that chain's block time; the comments on all three now state the wall-clock rationale. The value remains operator-overridable per deployment through the hub config overlay (`ACTIVATION_DELAY_BLOCKS` is in the indexer's `SCALAR_PARAMS`), so these are defaults, not hard limits.
- `src/actions/stake.js` / `src/actions/unstake.js` / `src/actions/delegate.js` — the activation-delay lookup in all eight staking handlers no longer hardcodes `6` as its last-resort fallback. Each site read `staking['ACTIVATION_DELAY_BLOCKS']` (the per-chain `STAKING` default) and fell back to a literal `6` if absent — silently applying BTC's value on LTC/DOGE if the chain config were ever incomplete, and duplicating the magic number across eight independent sites. The fallback now reads the chain-sourced `this.config['ACTIVATION_DELAY_BLOCKS']` (the hub overlay scalar) instead of a literal, so no path can substitute BTC's block count on another chain. Normal operation is unchanged — every chain config defines the `STAKING` value, which still takes precedence — and the stale "reuse the capability staking constant for now (6 blocks)" / "6-block delay for BTC reorg safety" comments were updated to reflect the per-chain calibration.
...
### Fixed
- `src/XChainIndexer.js` — `FEE_PAYMENT_MODE` is no longer applied from the hub-config overlay, removing a dead governance hook that advertised tunability the runtime never honoured. The field was listed in `SCALAR_PARAMS` (and published by `xchain-hub` as an operational param), so a hub operator could push a per-chain override and see it accepted and stored at both the hub and indexer config layers — but the runtime fee-mode detector `detectFeePaymentMode()` (`src/utility.js`) never reads `config['FEE_PAYMENT_MODE']`. It derives the mode purely from the coin (BTC falls back to XCHAIN balance deduction when no fee output is present; LTC/DOGE reject a missing fee output) and from whether the transaction actually pays the fee destination, so an override had zero observable effect on any of the action handlers that call the detector — a silent-misconfiguration hazard where following normal governance procedure produced no feedback that the change did nothing. `FEE_PAYMENT_MODE` is removed from `SCALAR_PARAMS` so the governance surface now matches actual runtime behaviour; the per-chain defaults in `configs/BTC.js`/`LTC.js`/`DOGE.js` (which document each chain's intended mode) and `detectFeePaymentMode()` itself are unchanged. The companion removal from `xchain-hub`'s published operational params keeps both sides of the surface consistent. No consensus path is touched. Any future tunability must be re-introduced with protocol-agreed activation-block gating, as already required for the other consensus-sensitive fee inputs.
- `src/configs/LTC.js` / `src/configs/DOGE.js` — `STAKING.CAPABILITIES` now declares an empty object `{}` instead of an empty array `[]`, matching the keyed-object shape used by `configs/BTC.js`. Capability staking is BTC-only at the protocol level, so LTC and DOGE correctly configure no capabilities — but the value was an empty array, a different runtime type than the capability map BTC declares. Property lookups behave identically on `[]` and `{}` (both return `undefined`), so the four capability-checking helpers (`isCapabilityConfigured`, `getValidatorsByCapability`, `getActiveCapabilityCount`, `hasCapability`) are unaffected today. The latent hazard is in `XChainIndexer._mergeHubParams()`: `STAKING` is listed in `BLOB_PARAMS` and the overlay performs a full-object replacement with no per-chain guard on its shape, so a hub that ever pushed a non-empty `CAPABILITIES` map for LTC/DOGE — via a misconfigured `updateconfig`, a hub bug, or a future governance change — would be silently accepted, and `isCapabilityConfigured()` would then return `true` on a chain where capability staking is structurally impossible. That would diverge the validator set from BTC indexers on consensus-sensitive paths (PBFT quorum, attestation responsible-set selection, rollback). Using an empty object makes the config structurally honest about its intent and consistent with BTC; no runtime behaviour changes and no callers are affected. The hub has never pushed a non-empty STAKING blob for LTC/DOGE, so there is no exposure on existing deployments — this hardens the shape before any governance change that touches STAKING config.
- `src/sql/index_addresses.sql` / `src/sql/migrations/unique_full_column_index_addresses.sql` — widened the `index_addresses` unique index from a 62-character prefix (`address(62)`) to the full `address` column. The column is `VARCHAR(120)`, so the prefix never matched the column width — full-column uniqueness was always the intent, and the matching `index_transactions`/`index_addresses` keys elsewhere in the platform already use full-column indexes. `db.js` upserts addresses with `INSERT IGNORE` + refetch, whose race-safety relies on the UNIQUE index being an exact key for the value inserted; a 62-char prefix index treats two distinct addresses sharing their first 62 characters as the same key, so the second `INSERT` is silently ignored and the caller refetches the *first* address's id, mis-mapping the new address to an existing id and propagating a corrupted address→id mapping downstream. All standard Bitcoin/Litecoin/Dogecoin addresses are ≤62 chars, so no live data is affected today; the fix removes the latent phantom-collision risk for any longer future address format. Fresh installs pick up the full-column index from `index_addresses.sql` via `verifyTables()`; the drift reconciler in `db.js` only reconciles columns and never indexes, so existing deployments must apply the new `migrations/unique_full_column_index_addresses.sql` manually (it swaps the prefix index for the full-column UNIQUE index). No dedup is required — the old index is already UNIQUE on the 62-char prefix, so every existing row already holds a distinct full address and the widening cannot fail on duplicate data. The migration is idempotent (`DROP INDEX IF EXISTS` / `CREATE UNIQUE INDEX IF NOT EXISTS`).
- `src/sql/pubkeys.sql` / `src/sql/addresses.sql` — normalized both files to the `DROP TABLE IF EXISTS <table>; CREATE TABLE <table> (…)` reset pattern used by every other table's DDL in this service. `pubkeys.sql` previously opened with `CREATE TABLE IF NOT EXISTS` and no preceding `DROP`; `addresses.sql` already had the `DROP` but still used `CREATE TABLE IF NOT EXISTS`. In both cases the `IF NOT EXISTS` meant that running the file directly against an existing table (a manual schema rebuild during disaster recovery or fresh-environment setup) silently no-opped or skipped the reset instead of recreating the table, masking a failed rebuild. `verifyTables()` skips already-existing tables, so neither path is exercised on a running service — the change is purely a manual-rebuild safety and consistency fix.
