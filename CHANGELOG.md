...
### Fixed
- `src/XChainIndexer.js` — a hub-pushed `GAS_SCHEDULE` governance update is now propagated to the live VM instance, closing a gas-metering consensus split. The `XChainVM` captures the gas schedule by reference at construction time (`this.gasSchedule = config.gasSchedule`) and hands that same reference to every `GasTracker` it creates for in-VM operations (`VM_STATE_READ`, `VM_EMISSION`, `VM_ATTEST_REQUEST`, …). When the hub-config overlay re-applied an updated `GAS_SCHEDULE`, `_mergeHubParams()` rebound `this.config['GAS_SCHEDULE']` to a freshly parsed object but left the VM pointing at the startup-time object, while the pre-VM base-fee charges (`VM_EXECUTE_BASE` / `VM_DEPLOY_BASE`) in `actions/execute.js` and `actions/deploy.js` read `this.config['GAS_SCHEDULE']` directly and picked up the new schedule immediately. The two charge sites therefore diverged within a single EXECUTE action after any governance update, and two honest nodes that last received the hub update at different times computed different total gas costs for the same transaction — a chain-split vector. `_mergeHubParams()` now re-points `this.actions.vm.gasSchedule` at the live config object after the overlay merge, so in-VM metering and the pre-VM base fees always read the same schedule. Updating the VM's instance property is sufficient because no other code path independently caches the reference; no VM re-instantiation is required. Covered by a new regression case in `test/unit/config.test.js`.
