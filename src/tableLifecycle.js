...
const TABLES = [
    ...
    { table: 'rollcalls', owner: 'indexer', replication: 'stream:block', rollback: 'block', replicaRollback: 'mirror',
      hashed: DERIVED,
      note: 'One row per epoch that reached its close block, INCLUDING unrolled ones. An unrolled epoch counts for nobody, but the K-streak must know which epochs to SKIP, and a missing row is indistinguishable from an epoch that has not closed yet.' },
    { table: 'rollcall_absences', owner: 'indexer', replication: 'stream:block', rollback: 'block', replicaRollback: 'mirror',
      hashed: DERIVED,
      note: 'One row per responsible SOURCE that did not sign at a rolled epoch, pinned at close and never re-derived (SLASH rewrites stakes.amount in place, so a later re-derivation can differ from the set the verdict was taken over). evicted = 1 is the rollback key the delegations repair clause keys on, because the eviction writes no DELEGATE-revoke row for the generic self-join repair to find.' },
