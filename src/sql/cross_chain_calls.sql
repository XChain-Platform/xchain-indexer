CREATE TABLE cross_chain_calls (
    id                    BIGINT NOT NULL AUTO_INCREMENT PRIMARY KEY, -- mirror cursor (matches hub id) + deterministic injection-order key
    call_id               VARCHAR(80)  NOT NULL,                   -- deterministic id derived in the source-chain VM run
    phase                 VARCHAR(10)  NOT NULL,                   -- 'dispatch' | 'result'
    snapshot_block        BIGINT NOT NULL,                         -- BTC-anchored block; selects the cross_chain validator set for sig verification
    network               VARCHAR(20)  NOT NULL,                   -- mainnet/testnet/regtest; signed into the canonical
    source_chain          VARCHAR(10)  NOT NULL,
    source_action_index   BIGINT NOT NULL,                         -- the XCALL v0 request row on the source chain (retraction key)
    source_contract_index BIGINT NOT NULL,
