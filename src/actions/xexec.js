...
class Xexec {
    ...
    async parse(params, data, error){
        ...
        // ── Verify 2f+1 cross_chain signatures over the dispatch canonical ───────
        ...
        if(validSigs < quorum){
            console.warn("\t XEXEC : call=" + String(c.call_id).substring(0,16) + '... : insufficient valid signatures (' + validSigs + '/' + quorum + ') — skipping');
            return;
        }
