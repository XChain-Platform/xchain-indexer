 ...
 **********************************************************************
 *
 * SWAP fees: the amount a create or edit owes for its expiration and any
 * ownership escrow, then the check that SOURCE can pay it (a native coin
 * output or an XCHAIN balance) and the balance debit that follows.
 *
 ********************************************************************/

// Price the swap: the expiration fee plus, on a create that escrows ownership, the
// ownership-escrow premium. Cancels are free.
async function priceSwap(handler, st){
    let { format, data, error, swapInfo, isOwnershipGive, fees } = st;
    ...
    if(!error && (format==0 || format==2)){
        ...
        if(unifiedFees){
            ...
            if(!handler.util.isNull(data['EXPIRATION'])){
                let exp = handler.util.getUnifiedExpirationFee(data, swapInfo);
            ...
            fees['AMOUNT'] = handler.util.getExpirationFee(data, swapInfo);
        }
    }
}
...
module.exports = { priceSwap, validateFeePayment };
