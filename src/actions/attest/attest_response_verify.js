 ...
 * pinned by test/unit/actions/attest/attest_response_verify_vectors.test.js, whose
...
async function verifyAttestationResponse(input){
    ...
    let { declaredBlock, snapshotBlock } = verifyHeights(request, atBlock, network);

    let built        = await buildVerifyCanonical(input, { requestId, requestIdRaw, providerId,
                           responseStatus, meta, responseBodyBytes, effectiveTime, declaredBlock,
                           gateBlock, network, protocolChanges, error });
    let responseHash = built.responseHash;
    ...
    if(!error){
        let capableSet = await capableSignerSet(indexerDb, coin, network, declaredBlock, snapshotBlock);
        verifiedSigs   = await verifyEligibleSigs(sigs, capableSet, canonical, indexerDb, snapshotBlock);

        let filtered = await filterToResponsibleSet({ verifiedSigs, request, requestId, atBlock,
                                                      declaredBlock, network, computeResponsibleSet, error });
        verifiedSigs = filtered.verifiedSigs;
        validSigs    = filtered.validSigs;
...
function verifyHeights(request, atBlock, network){
    ...
    return { declaredBlock, snapshotBlock };
}
...
async function buildVerifyCanonical(input, f){
    let { requestId, requestIdRaw, providerId, responseStatus, meta, responseBodyBytes,
          effectiveTime, declaredBlock, gateBlock, network, protocolChanges } = f;
    let error = f.error;
    let responseHash = crypto.createHash('sha256').update(responseBodyBytes).digest('hex');
    let canonId  = await canonicalRequestId(protocolChanges, gateBlock, requestId, requestIdRaw);
    ...
    return { responseHash, canonical, error };
}
...
async function canonicalRequestId(protocolChanges, gateBlock, requestId, requestIdRaw){
    return (await protocolChanges.isEnabled('ATTEST_CANONICAL_LOWERCASE_ID', gateBlock))
...
async function capableSignerSet(indexerDb, coin, network, declaredBlock, snapshotBlock){
...
// validators signed.
async function verifyEligibleSigs(sigs, capableSet, canonical, indexerDb, snapshotBlock){
    let verifiedSigs = [];
...
//
// Returns the surviving signatures, their count, and the quorum verdict over them.
async function filterToResponsibleSet(p){
    let { verifiedSigs, request, requestId, atBlock, declaredBlock, network,
          computeResponsibleSet } = p;
    let error     = p.error;
    let validSigs = 0;
    ...
    return { verifiedSigs, validSigs, error };
}
module.exports = {
