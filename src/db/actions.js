...
// getBlockHashes and its gathering stay here: both blockhash conformance guards read the
// consensus SQL out of this file's text, and xchain-sync's copy reads this exact path. The
// gathering is one method per query, declared in gathering order (credits, debits, escrows,
// actions, contracts, contract state, executions, emissions, deposits, withdrawals, then the
// previous-block hashes), which is the order the guards compare the literals in.
...
module.exports = {
    ...
    async getBlockHashes(block_index){
        ...
        const ledger         = await this.getBlockHashLedgerRows(block_index);
        const actions        = await this.getBlockHashActionRows(block_index);
        const contracts_data = await this.getBlockHashContractData(block_index);
        ...
        const hashes = await this.getPreviousBlockHashes(block_index - 1);
        let info = this.foldBlockHashes(block_index, ledger, actions, contracts_data, hashes);
        ...
        // class never hashed.
        info['state'] = [];
        info['state']['hash'] = this.util.getDataHash(await this.buildBlockStateHashData(block_index));
        return info;
    },

    // Ledger rows for the ledger hash: credits, debits and escrows, gathered in that
    // order and canonicalized before anything hashes them.
    async getBlockHashLedgerRows(block_index){
        // Placeholer for ledger data (credits + debits + escrows)
        let ledger  = {
            credits:  await this.getBlockHashCreditRows(block_index),
            debits:   await this.getBlockHashDebitRows(block_index),
            escrows:  await this.getBlockHashEscrowRows(block_index)
        };
    ...
    async getBlockHashCreditRows(block_index){
        ...
        return this.doQuery(query, [block_index]);
    },
    ...
    // Contract hash data
    async getBlockHashContractData(block_index){
        let contracts_data = {
            contracts:   await this.getBlockHashContractRows(block_index),
            state:       await this.getBlockHashContractStateRows(block_index),
            executions:  await this.getBlockHashExecutionRows(block_index),
            emissions:   await this.getBlockHashEmissionRows(block_index),
            deposits:    await this.getBlockHashDepositRows(block_index),
            withdrawals: await this.getBlockHashWithdrawalRows(block_index)
        };
    ...
    async getBlockHashContractRows(block_index){
        let query = `SELECT c.action_index, a1.address AS source_address, c.code_hash, s1.status AS status
                 FROM contracts c
    ...
    async getBlockHashExecutionRows(block_index){
        let query = `SELECT ce.action_index, ce.contract_index, a1.address AS caller_address, ce.gas_used, s1.status AS status, ce.emitted_count
    ...
    async getBlockHashDepositRows(block_index){
        let query = `SELECT d.action_index, d.contract_index, a1.address AS source_address, t1.tick AS tick, d.amount, s1.status AS status
                 FROM deposits d
    ...
    async getBlockHashWithdrawalRows(block_index){
        let query = `SELECT w.action_index, w.contract_index, a1.address AS source_address, t1.tick AS tick, w.amount, s1.status AS status
                 FROM withdrawals w
    ...
    // Fold the gathered rows into the three chained block hashes. The tail below is
    // the hash-assembly shape both conformance guards compare against the follower.
    foldBlockHashes(block_index, ledger, actions, contracts_data, hashes){
        let info    = [];
    ...
    // The state_hash preimage (see the note at the call site in getBlockHashes).
    async buildBlockStateHashData(block_index){
        ...
        return buildStateHashData(this, block_index, {
