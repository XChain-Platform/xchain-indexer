 ...
 * characters; below it the legacy 5-character bound is preserved verbatim so
 * replay stays bit-identical.
 *
 * CONSENSUS. Below the gate a round carrying XCHAIN/USD is invalid on every node;
 * at/above it, valid on every node. A one-sided deploy forks the fleet on the
 * first such round.
 *
 ...
 * height names one cutover (same rationale protocol_changes.js records for
 * FIX_OUTPUT_FANOUT).
 *
 * Only the ticker side widens: no FIAT_CODE exceeds 3 characters (constants.js
 * VALID_FIAT_CODES), so widening the fiat side would admit nothing legitimate.
 *
 * No per-entry-drop behavior: a malformed pair invalidates the whole action,
 * before and after this gate. The pair set is inside the signed payload
 * (ed25519.buildPriceV0Payload reconstructs the signable bytes FROM the parsed
 * pairs), so skipping one entry would change the payload, fail every signature,
 * and reject the round anyway with a less informative error. A signed round is
 * atomic; this gate changes exactly one thing, which pairs are well-formed.
 *
 * Deploy order is part of this gate: widen every hub and every indexer FIRST,
 * while nothing proposes the pair; only then may a leader include XCHAIN/USD in
 * a round. Reversed, the hub's co-sign whitelist and round ingest reject the
 * whole round, which stalls all 36 pairs federation-wide and fails fee
 * validation on every chain once the 1800s staleness bound elapses.
 *
 * LOCAL COPY of the canonical map in xchain-documentation/protocol/constants.js
 * (PRICE_PAIR_WIDEN_ACTIVATION); kept value-identical by
 * test/unit/activationConstantsParity.test.js. A one-sided edit forks PRICE v0
...
// scheduled flag-day. The pre-launch instant this arms at is an open operator
// decision; the usual contract-era stamp 1786060800 (2026-08-07) is not usable
// because it falls AFTER the early-September launch target and would leave
