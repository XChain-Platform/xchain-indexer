...
class Mapper {
    ...
    async createMappings(data){
        ...
        await this.writeAddressTickMappings(action_index);
        ...
        if(action=='LINK' && status=='valid')
            await this.linkFileToIssueMapping(data);
    }
    ...
    async writeAddressTickMappings(action_index){
    ...
    // Process any FILE->ISSUE links for a valid LINK action.
    async linkFileToIssueMapping(data){
