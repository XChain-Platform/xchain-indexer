...
// fresh import target already holds it; re-inserting would collide. The target's
// schema setup likewise owns its schema_migrations ledger.
const EXCLUDE_TABLES = new Set(['blocks', 'index_fiats', 'schema_migrations']);
    ...
    async importDumpRows(file){
        ...
        for await (let line of rl){
            ...
            if(obj.t){
                ...
                skipTable = obj.t === 'schema_migrations';
                continue;
            }
            if(obj.r){
    ...
    async verifyImportedBlockHashes(meta){
        ...
        for(let k of ['ledger', 'actions', 'state', 'contracts']){
            if(got[k] !== meta.expectedHashes[k]){
                getLogger().error('GENESIS FATAL: imported ' + k + ' hash ' + got[k] +
