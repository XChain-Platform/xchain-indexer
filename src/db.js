...
class Database {
    ...
    async verifyTables(){
        ...
        for (file of files){
            ...
            if(isSql){
                ...
                let query   = `SELECT * FROM information_schema.tables WHERE table_schema = ? AND table_name = ?`;
                let args    = [this.dbName, table];
    ...
    async getBlockHashes(block_index){
        ...
        query = `SELECT
                ...
                ORDER BY 
                    c.action_index ASC`;
        ledger.credits = await this.doQuery(query, args);
        ...
        query = `SELECT
                ...
                ORDER BY 
                    d.action_index ASC`;
        ledger.debits = await this.doQuery(query, args);
        ...
        query = `SELECT
                ...
                ORDER BY 
                    e.action_index ASC`;
        ledger.escrows = await this.doQuery(query, args);
        ...
        query = `SELECT
                ...
                ORDER BY 
                    a.action_index ASC`;
        actions = await this.doQuery(query, args);
    ...
    async createBlock(block_index, block_time){
        ...
        let args  = [block_time, ledger_hash_id, actions_hash_id, block_index];
        if(block_id!=null){
            query = `UPDATE
                        blocks
                    SET
        ...
        let results = await this.doQuery(query, args);
        // Shorten up hashes to the first 5 characters
    ...
    async createTxIndex(data){
        ...
        if(tx_index==null){
            ...
            let args        = [tx_index, block_index, tx_hash_id, source_id];
            let results     = await this.doQuery(query, args);
        }
        return tx_index;
    }
    ...
    async getTokenSupplyToken(tick){
        ...
        if(results.length > 0 && !this.util.isNull(results[0].supply))
            supply = Number(results[0].supply);
        return supply;
    }
    ...
    async sanityCheck(block_index){
        ...
        let query   = `SELECT
                    ...
                    ORDER BY 
                        t2.tick ASC`;
        let args    = [block_index, block_index, block_index];
        let results = await this.doQuery(query, args);
        if(results.length >0){
            for(let row of results){
    ...
    async getSwapInfo(coin, action_index){
        ...
        let query = `SELECT 
                    ...
                    FROM 
                        ...
                        INNER JOIN swap_statuses   s2 ON (s2.swap_action_index = s.action_index)
                    WHERE 
                        s2.action_index = (
                            SELECT
    ...
    async getOrderInfo(coin, action_index){
        ...
        let query = `SELECT 
                    ...
                    FROM 
                        ...
                        INNER JOIN order_statuses  s2 ON (s2.order_action_index=o.action_index)
                    WHERE 
                        s2.action_index = (
                            SELECT
                                MAX(s_inner.action_index)
