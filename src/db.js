...
class Database {
    ...
    async createLedgerChangeRecord(table, action_index, tick, amount, address){
        ...
        let exists = false;
        try {
            let results = await this.doQuery(query, [action_index, address_id, tick_id]);
            if(results.length > 0)
                exists = true;
        } catch (error){
    ...
    async updateAddressBalance(address, rollback){
        ...
        for(let tick_id in balances){
            ...
            try {
                let results = await this.doQuery(query, [address_id, tick_id]);
                if(results.length > 0)
                    action = 'update';
            } catch (error){
    ...
    async createAddressOption(data){
        ...
        try {
            let results = await this.doQuery(query, [fee_preference, require_memo, memo_id, status_id, action_index]);
        } catch (error){
