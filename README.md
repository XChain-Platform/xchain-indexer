...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/tests-9%2C239%2B%20passing-brightgreen" alt="Tests">
...
- **Hub-facing RPCs**: `getownstake`, `getactivevalidators`, `getactivestakeweights`, `getcapabilityvalidators`, `getstakeweightsbycapability`, `getfullnodeverifiers`, `getpendingattestation_requests`, `getrelayedattestation_requests`, `getopencrosschainorders`, `getpendingcrosschaincalls`, `getcrosschaincall`, `getcrosschaincallresult`, `getactionconfirmations`, `getpricebatches`, `getstakesourcebypubkey`, `getlatestblock`, `getblockhashes`. `pushvalidatorrewards` is retired: it refuses every reward type, because every validator reward is derived from on-chain bytes
...
| `npm test` | Run unit tests (~8,536 tests) |
| `npm run test:integration` | Integration tests (~284 tests, requires MariaDB) |
...
| `npm run test:security` | Security tests (123 tests) |
...
| `npm run test:regression` | Regression tests (tagged across all suites, 7,698 tests) |
| `npm run test:regression:fast` | Fast regression (tier1 + tier4, unit only, 3,729 tests) |
| `npm run test:regression:full` | Full regression suite (7,698 tests) |
| `npm run test:nodb` | All tests that don't require a database (8,789 tests) |
| `npm run test:full` | Complete test suite (9,143 tests) |
