...
//
// The Config suite (getConfig per coin) lives here. The hub config overlay suite and its
// env-override blocks live beside it in config.test/, each file opening the same
// 'XChainIndexer hub config overlay' describe so every full test title is unchanged;
// config.test/helpers/overlay_indexer.js holds the indexer builder those blocks share.

const assert = require('assert');

// A fresh BTC regtest getConfig(), loaded the way the 'getConfig() - BTC' before hook
// always has: env set, config.js purged from the require cache, then required again.
// Each of the four sibling BTC blocks calls it, so all four read the same config.
