...
function registerAdvisoryPinTests() {
...
function testMinimatchBraceExpansion() {
...
function testBraceExpansionBounds() {
    this.timeout(30000);
...
describe('Security: remediated dependency advisories @regression @tier4', function () {
    registerAdvisoryPinTests();
    it('ADV-3: minimatch can still brace-expand through the overridden brace-expansion', testMinimatchBraceExpansion);
    it('ADV-4: brace-expansion survives the CVE-2026-14257 unbounded-length input', testBraceExpansionBounds);
    it('ADV-5: the installed axios reports a patched runtime version', testInstalledAxios);
    it('ADV-10: the installed mariadb reports a patched runtime version', testInstalledMariadb);
});
