...
function makeXcallRequestRow(overrides = {}) {
    return {
...
function setupXcall() {
    ...
    db.getCrossChainCallRequestById      = sinon.stub().resolves(makeXcallRequestRow());
...
// ---------------- XCALL ----------------
function registerXcallExpiryTests() {
...
function registerXcallResultTests() {
...
function registerAttestMixedTests() {
    // Legacy COUNT path for the responsible set (matches attest.test.js default).
    // And the response-mirror flag day, armed on regtest at genesis: above it the
    // chain handler refuses an on-chain v1 outright, so a callback is never
    // injected and there is no fault to propagate. These cases are the legacy
...
function registerAttestExpiryTests() {
    // Legacy COUNT path for the responsible set (matches attest.test.js default).
    // And the response-mirror flag day, armed on regtest at genesis: above it the
    // chain handler refuses an on-chain v1 outright, so a callback is never
    // injected and there is no fault to propagate. These cases are the legacy
...
describe('Callback injection infra-fault propagation @regression @tier2', function () {
    ...
    registerXcallExpiryTests();
    registerXcallResultTests();
    registerAttestResponseFaultTests();
    registerAttestMixedTests();
    registerAttestExpiryTests();
});
