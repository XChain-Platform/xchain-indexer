...
//
// Amount format validation and SEND action-path coverage. One part of
// amount_representability.test.js; the shared fixtures are in
// helpers/amount_representability_suite.js.
...
} = require('./helpers/amount_representability_suite.js');

let util;

function acceptedRepresentableAmountCases() {
    const ACCEPTED = [
...
function validatorCompatibilityCases() {
    beforeEach(function () { util = new Utility(); });
...
describe('Amount representability @regression @tier1', function () {
    describe('isRepresentableAmount() rule', acceptedRepresentableAmountCases);
    describe('isValidAmountFormat(): the credited number vs the validated text', creditedNumberCases);
    describe('isValidAmountFormat(): the credited number vs the validated text', validatorCompatibilityCases);
    describe('SEND: the gate reaches a real action path', sendGateCases);
});
