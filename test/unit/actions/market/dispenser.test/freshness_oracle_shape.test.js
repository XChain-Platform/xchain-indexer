...
const SHAPE_ROW   = 'dispenser_freshness_shape_activation.DISPENSER_FRESHNESS_SHAPE_ACTIVATION';
const ARMED_AT    = 400;   // below the 961000 freshness height, so the tracker path still runs

let origFetch;

// Arms the oracle-shape row at `height` on mainnet BTC for the current test only.
// The registry row is frozen (mainnet ships UNARMED), so the handler's read of it
// is stubbed instead of the table edited; the harness's sinon.restore() disarms it.
function armShapeAt(height) {
    stubActiveAt(sinon, SHAPE_ROW, false).withArgs(SHAPE_ROW)
        .callsFake((key, network, coin, blockIndex) =>
            network === 'mainnet' && coin === 'BTC' && Number(blockIndex) >= height);
}
...
describe('Dispenser action handler @regression @tier2', function () {
    ...
    describe('GET_ADDRESS different from SOURCE validation (freshness)', function () {
        ...
        //    client (the dispenser_freshness_shape_activation row). A non-null
        ...
        describe('oracle-shape flag-day (malformed tracker reply must not grant freshness)', function () {
            ...
            afterEach(function () { global.fetch = origFetch; });
