 ...
 *   npx mocha --no-config --exit test/integration/scenarios/controller_stake/25_controller_bulk_transfer_gating.test.js
 ********************************************************************/
'use strict';
...
async function setupControllerSuite() {
...
async function teardownControllerSuite() {
    if (indexer) await destroyIndexer(indexer);
    await destroyFileIndexers(__filename);
    await closeAll();
}
async function testControllerSetup() {
...
// --- AIRDROP ---
async function testDeniedAirdrop() {
...
async function testAllowedAirdrop() {
...
// --- DIVIDEND ---
async function testDeniedDividend() {
...
async function testAllowedDividend() {
...
// --- SWEEP ---
async function testDeniedSweep() {
...
async function testAllowedSweep() {
...
describe('Controller: AIRDROP/DIVIDEND/SWEEP gated by the transfer controller (real DB + real VM) @phaseE', function () {
    this.timeout(600000);
    before(setupControllerSuite);
    after(teardownControllerSuite);
    it('set up tokens, guards, address list, and distributions', testControllerSetup);
    it('a transfer-class DENY reverts an AIRDROP (no recipient credited)', testDeniedAirdrop);
    it('a transfer-class ALLOW commits an AIRDROP and the block passes the supply sanity check', testAllowedAirdrop);
    it('a transfer-class DENY reverts a DIVIDEND (no holder credited)', testDeniedDividend);
    it('a transfer-class ALLOW commits a DIVIDEND and the block passes the supply sanity check', testAllowedDividend);
    it('a transfer-class DENY on any swept tick fails the WHOLE sweep (fail-closed)', testDeniedSweep);
    it('a transfer-class ALLOW commits a SWEEP of the controlled tick and the block passes sanity', testAllowedSweep);
});
