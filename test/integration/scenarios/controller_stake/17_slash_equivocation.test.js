...
async function setupSlashSuite() {
...
async function teardownSlashSuite() {
    await destroyFileIndexers(__filename);
    await closeAll();
}
async function testResolvedSnapshotQualification() {
...
function testSlashRowsRecorded() {
...
async function testDeterministicHashChain() {
...
async function testPermanentDisqualification() {
...
async function testConfigEquivocation() {
    const SEQ = '5', VIEW = 0;
...
describe('Integration: live SLASH equivocation drill + determinism @regression @tier1', function () {
    this.timeout(120000);
    before(setupSlashSuite);
    after(teardownSlashSuite);
    it('the offender qualified for cross_chain at the RESOLVED snapshot height before the slash (sanity)', testResolvedSnapshotQualification);
    it('the equivocation proof burns the offender\'s entire capability bond', testEntireBondBurned);
    it('records exactly one capability_slash_event + a verbatim stakes debit', testSlashRowsRecorded);
    it('re-deriving from a clean DB yields the IDENTICAL hash chain (determinism = no fork)', testDeterministicHashChain);
    it('a slashed key stays disqualified after a fresh re-stake from a new source (global + permanent)', testPermanentDisqualification);
    it('an XCONFIG equivocation burns the bond via whole-federation membership', testConfigEquivocation);
});
