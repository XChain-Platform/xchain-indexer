 ...
 *   npx mocha --no-config --exit test/integration/scenarios/controller_stake/20_cooldown_completion_supply.test.js
 ********************************************************************/
'use strict';
...
// The chain both tests read: gas bootstrap, a STAKE that bonds 6000 XCHAIN and the UNSTAKE
// that schedules the cooldown, all indexed by the real indexer.
async function seedStakeAndUnstake() {
...
async function closeIndexerAndSchemas() {
...
async function completeCooldown(ce) {
...
async function assertCreditAttributedToCooldownBlock(ce) {
...
// Reads the three GAS supplies and asserts they agree; why all three is at the call site.
...
async function assertThreeWaySupply() {
...
describe('Unstake cooldown completion: GAS supply conservation (real DB + real VM) @phaseE', function () {
    this.timeout(600000);
    before(seedStakeAndUnstake);
    after(closeIndexerAndSchemas);
    ...
    it('cooldown completion credits the bond back AND keeps GAS supply == ledger (no SanityError)', async function () {
        ...
        await assertCreditAttributedToCooldownBlock(ce);
