 ...
 *
 * The DISPENSER cases live in 03_order_dispenser.test/dispenser.test.js under this file's
 * describe title; the shared actors, tokens and fresh-indexer helper are in
 * 03_order_dispenser.test/helpers/order_book_chain.js.
 */
...
const { ADDR1, ADDR2, TICK_A, TICK_B, T0, T_FAR_FUTURE, freshIndexer } = require('./03_order_dispenser.test/helpers/order_book_chain');
...
// ORDER TESTS
// ===========================================================================
// The chain the "ORDER format 0 – create order" tests read: seeded and processed once, exactly as that
// block's own before hook did before the body moved here.
async function seedORDERFormat0CreateOrder() {
...
// The chain the "ORDER_MATCH – two complementary orders match" tests read: seeded and processed once, exactly as that
// block's own before hook did before the body moved here.
async function seedORDERMATCHTwoComplementaryOrders() {
    const { seeder, indexer } = await freshIndexer();
...
// The chain the "ORDER format 1 – cancel order" tests read: seeded and processed once, exactly as that
// block's own before hook did before the body moved here.
async function seedORDERFormat1CancelOrder() {
    const { seeder, indexer } = await freshIndexer();
    ...
    await processBlocks(indexer);
    const orderActionIndex = await helpers.getLastActionIndexByType(indexerQuery, 'ORDER');
    ...
    return { indexer, orderActionIndex };
}

// The chain the "ORDER expiration – order expires when block_time passes EXPIRATION" tests read: seeded and processed once, exactly as that
// block's own before hook did before the body moved here.
async function seedORDERExpirationOrderExpiresWhen() {
    const { seeder, indexer } = await freshIndexer();
...
describe('03 ORDER / DISPENSER integration @regression @tier2', function () {
    ...
    describe('ORDER format 0 – create order', function () {
        ...
        before(async function () { indexer = await seedORDERFormat0CreateOrder(); });
...
describe('03 ORDER / DISPENSER integration @regression @tier2', function () {
    ...
    describe('ORDER_MATCH – two complementary orders match', function () {
        ...
        before(async function () { indexer = await seedORDERMATCHTwoComplementaryOrders(); });
...
describe('03 ORDER / DISPENSER integration @regression @tier2', function () {
    ...
    describe('ORDER format 1 – cancel order', function () {
        ...
        before(async function () { ({ indexer, orderActionIndex } = await seedORDERFormat1CancelOrder()); });
    ...
    describe('ORDER expiration – order expires when block_time passes EXPIRATION', function () {
        ...
        before(async function () { indexer = await seedORDERExpirationOrderExpiresWhen(); });
